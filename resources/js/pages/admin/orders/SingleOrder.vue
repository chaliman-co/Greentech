...
onMounted(async () => {
  if (!order.value || order.value.id !== route.params.id) {
    const orderFromStore = store.state.orders.data.find(order => order.id === Number(route.params.id))
    if (orderFromStore) store.commit("set_current_order", orderFromStore)
    else try {
      const response = await getFromApi(`/orders/${route.params.id}`);
      if (response.failed) handleErrors(response)
      else {
        store.commit("set_current_order", response.data)
      }
    } catch (err) {
